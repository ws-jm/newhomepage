...
//				print_r($_FILES);
    ...
    $user_unique_id = $_POST['unique_id'];
    ...
    $stmt3->bind_param("s", $user_unique_id);
    $stmt3->execute();
    $stmt3->bind_result($user_credit);
    $stmt3->fetch();
    $conn3->close();

    if (($user_credit != 0 || !empty($user_credit)) && $_POST['no_of_sms'] < $user_credit) {
        ...
        if (empty($_SESSION['error'])) {
            ...
            $per_sms_price = isset($_POST['per_sms_price']);
            ...
            $stmt->bind_param("dssss", $no_of_sms, $unique_id, $user_unique_id, $txn_type, $created_date);
            if ($stmt->execute()) {
                ...
                if ($stmt1->execute()) {
                    ...
                    $stmt2->bind_param("dss", $no_of_sms, $updated_at, $user_unique_id);
                    $stmt2->execute();
                    $conn2->close();
                }
                $conn1->close();
