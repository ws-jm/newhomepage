...
if (isset($_REQUEST['submit'])) {
    ...
    if (($credit != 0 || !empty($credit)) && $_REQUEST['no_of_sms'] < $credit) {

        if (isset($_REQUEST['no_of_sms']) && empty($_REQUEST['no_of_sms'])) {
            $_SESSION['error'] .= "<li>No of SMS should not be blank</li>";
        }
        if (isset($_REQUEST['per_sms_price']) && empty($_REQUEST['per_sms_price'])) {
        ...
        if (empty($_SESSION['error'])) {
            $txn_type = $_REQUEST['action'];
            $user_unique_id = $_REQUEST['unique_id'];
            $no_of_sms = $_REQUEST['no_of_sms'];
            $description = $_REQUEST['description'];
            $per_sms_price = $_REQUEST['per_sms_price'];
            $total_amount = $no_of_sms * $per_sms_price;
            $tax_amount = 0;
            $unique_id = $_SESSION['user_unique_id'];


            if (isset($_REQUEST['is_tax']) && ($_REQUEST['is_tax']) == 'true') {      
                $tax_status = $_REQUEST['is_tax'];
                $total_amt = $no_of_sms * $per_sms_price;
                $tax_amount = ($total_amount * 18) / 100;
                $total_amount = $total_amt + $tax_amount;
            }

            if(isset($_REQUEST['is_tax']) == "true"){
                $tax_status = "Yes";
            ...
            $stmt->bind_param("dddsiddsssss", $no_of_sms, $per_sms_price, $credit, $tax_status, $tax_percentage, $tax_amount, $total_amount, $description, $unique_id, $user_unique_id, $txn_type, $created_date);
            if ($stmt->execute()) {
                ...
                $stmt1->bind_param("dss", $no_of_sms, $updated_at, $user_unique_id);
                if ($stmt1->execute()) {
                    $conn2 = new mysqli($servername, $username, $password, $dbname);
                    if ($conn2->connect_error) {
                        die("Connection failed: " . $conn2->connect_error);
            ...
            header("Location: user.php?added");
            exit();
        } else {
