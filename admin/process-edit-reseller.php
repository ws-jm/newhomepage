            ...
            if (isset($_FILES['photo']) && !empty($_FILES['photo']['size'])) {
                ...
                if ($count_id == 1) {
                    ...
                    if (!empty($pimagename)) {
                        ...
                        $stmt = $conn->prepare("Update logins Set full_name = ?, username = ?, email_id = ?, pwd = ?, mobile = ?, company = ?, profilepic = ?, status = ?,rollback= ?, updated_at = ? where user_unique_id = ? and  user_type = ?");
                        $stmt->bind_param("ssssssssssss", $full_name, $user_name, $email_id, $pwd, $mobile, $company, $pimagename, $status,$rollback, $update_date, $user_unique_id, $user_type);
                        $stmt->execute();
                        $conn->close();
                        unset($_SESSION['error']);
                        header("Location: reseller.php?edited");
                        exit();
                    }
                }else{
                    $conn = new mysqli($servername, $username, $password, $dbname);
                ...
                $stmt = $conn->prepare("Update logins Set full_name = ?, username = ?, email_id = ?, pwd = ?, mobile = ?, company = ?, status = ?,rollback=?, updated_at = ? where user_unique_id = ? and  user_type = ?");
                $stmt->bind_param("sssssssssss", $full_name, $user_name, $email_id, $pwd, $mobile, $company, $status,$rollback,$update_date, $user_unique_id, $user_type);
                $stmt->execute();
                $conn->close();
                unset($_SESSION['error']);
                header("Location: reseller.php?edited");
